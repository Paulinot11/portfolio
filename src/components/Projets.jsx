        ...
        <section id="projets" className="py-20">
            <div className="container-custom">
                <h2 className="section-title">
                    <FaFolderOpen className="inline mr-3 text-accent" /> Projets académiques
                </h2>
                
                {/* Filtres */}
                <div className="flex flex-wrap gap-3 justify-center mb-10">
                    <FaFilter className="text-accent text-xl mr-2 self-center" />
                    {allTechs.map(tech => (
                        <button
                            key={tech}
                    ...
                    {projetsFiltres.map(projet => {
                        ...
                        return (
                            <div key={projet.id} className="bg-card-bg p-6 rounded-2xl border border-border card-hover">
                                ...
                                <h3 className="text-xl font-bold mb-3 text-text-primary">{projet.titre}</h3>
                                <p className="text-text-secondary mb-4">{projet.description}</p>
                ...
                <h2 className="section-title mt-20">
                    <FaBriefcase className="inline mr-3 text-accent" /> Stage réalisé
                </h2>
                <div className="bg-card-bg p-6 md:p-8 rounded-2xl border-l-4 border-accent">
                    <div className="flex flex-wrap justify-between items-start mb-4">
                        <h3 className="text-xl font-bold text-text-primary">
                            <FaCode className="inline mr-2 text-accent" /> {stage.titre}
                        </h3>
                        <span className="text-accent"><FaCalendarAlt className="inline mr-2" /> {stage.date}</span>
                    </div>
                    <p className="mb-4 text-text-secondary">
                        <FaTag className="inline mr-2 text-accent" /> <strong>Thème :</strong> {stage.theme}
                    </p>
                    <div className="flex flex-wrap gap-2">
